...
exports.action = async function () {
  ...
  try {
    ...
    if (await fs.pathExists(`${folder}/dfour.yaml`)) {
      valid = true
      workspace = core.getInput('dfour_workspace')
      endpoint = core.getInput('dfour_endpoint')
      process.env.DFOUR_ENDPOINT = endpoint
      process.env.DFOUR_USERNAME = core.getInput('dfour_username')
      process.env.DFOUR_PASSWORD = core.getInput('dfour_password')
    }
  } catch (error) {
  ...
  if (valid) {
    try {
      ...
      const { stdout } = await promExec(`dfour workspace -y ${workspace} ${folder}`)
